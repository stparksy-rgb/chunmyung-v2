...
with st.sidebar:
    ...
    with col1:
        birth_year = st.number_input("년", min_value=1900, max_value=2100, value=1969, label_visibility="collapsed")
        st.caption("년")
    with col2:
        birth_month = st.number_input("월", min_value=1, max_value=12, value=8, label_visibility="collapsed")
        st.caption("월")
    with col3:
    ...
    with col4:
        birth_hour = st.number_input("시", min_value=0, max_value=23, value=13, label_visibility="collapsed")
        st.caption("시")
    with col5:
        birth_minute = st.number_input("분", min_value=0, max_value=59, value=30, label_visibility="collapsed")
        st.caption("분")
...
if st.session_state.saju_calculated and st.session_state.saju:
    ...
    if st.session_state.messages and st.session_state.messages[-1]["role"] == "user":
        with st.spinner("🔮 천명 VIP가 사주를 분석하고 있습니다..."):
            context = generate_saju_context(saju, is_lunar, birth_year)
