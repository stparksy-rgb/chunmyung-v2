...
통합 버전 v2.0 - Claude API 연동 + 채팅 기능
...
def generate_saju_context(saju):
    ...
    for i, (name, pillar, pillar_kr) in enumerate(pillars):
        with cols[i]:
            ...
            <div style='background: rgba(255,255,255,0.05); border: {border}; border-radius: 15px; padding: 15px; text-align: center;'>
                ...
                <div style='color: #888; font-size: 0.9rem;'>{pillar_kr}</div>
            </div>
            """, unsafe_allow_html=True)
    
    # 일간 & 오행
    st.markdown(f"""
    **일간(나):** {saju['day_gan_kr']} ({CHEONGAN_OHAENG[saju['day_gan']]}) | 
    **오행분포:** 木 {saju['ohaeng_count']['木']} | 火 {saju['ohaeng_count']['火']} | 土 {saju['ohaeng_count']['土']} | 金 {saju['ohaeng_count']['金']} | 水 {saju['ohaeng_count']['水']}
    """)
    ...
    quick_questions = [
        ...
        ("❤️ 연애운", "이 사주의 연애운과 결혼운을 분석해주세요. 어떤 배우자가 맞고, 올해 연애운은 어떤가요?"),
        ...
        for msg in st.session_state.messages:
            if msg["role"] == "user":
                ...
                <div class="chat-message user-message">
                    <strong>🙋 질문:</strong><br>{msg["content"]}
                </div>
                """, unsafe_allow_html=True)
            else:
                st.markdown(f"""
                <div class="chat-message assistant-message">
                    <strong>🔮 천명 VIP:</strong><br>
                </div>
                """, unsafe_allow_html=True)
    ...
    if st.session_state.messages and st.session_state.messages[-1]["role"] == "user":
        with st.spinner("🔮 천명 VIP가 사주를 분석하고 있습니다..."):
            saju_context = generate_saju_context(saju)
...
# =====================================================
    ...
    # 서비스 특징
    st.markdown("### 🌟 천명 VIP v2.0 새로운 기능")
    ...
    with col2:
        st.markdown("""
        <div style='background: rgba(255,255,255,0.05); padding: 20px; border-radius: 15px; height: 180px;'>
            <h4 style='color: #ffd700;'>🤖 AI 자동 통변</h4>
            <p style='color: #ccc;'>
            버튼 하나로 VIP급 심층 분석.
            종합운/직업운/재물운/연애운/건강운
            </p>
    ...
    st.markdown("---")
    st.info("💡 **Tip:** API 키는 Streamlit Cloud > Settings > Secrets에서 `ANTHROPIC_API_KEY`로 설정하세요.")
